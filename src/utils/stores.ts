...
export const getStores = async ({ lng, lat }: Coords) => {
  const res = await fetch(
    `https://taste-buds-iota.vercel.app/api/stores/?lng=${lng}&lat=${lat}`
  );
...
export const getStore = async (store_id: string) => {
  const res = await fetch(`https://taste-buds-iota.vercel.app/api/stores/${store_id}`);
  const data = res.json();
  //console.log(data);
  return data;
};
    ...
    if (user_keeps.length >= 1) {
      ...
      const storePromises = user_keeps.map(async (item: string) => {
        const response = await fetch(
          `https://taste-buds-iota.vercel.app/api/stores/${item}`
        );
